...
frts::CarveAction::CarveAction(const IdPtr& jobId, const IdPtr& jobType, const IdUnorderedSet& carveEntities,
                               bool doDrop, bool fromAbove, const IdUnorderedSet& harvestTypes,
                               const IdUnorderedSet& jobRequirements, const IdPtr& jobMarker)
    : jobId{jobId}, jobType{jobType}, carveEntities{carveEntities}, doDrop{doDrop}, fromAbove{fromAbove},
      harvestTypes{harvestTypes}, jobRequirements{jobRequirements}, jobMarker{jobMarker}
{

}

frts::CarveAction::~CarveAction()
{

}

frts::Action::State frts::CarveAction::execute(const SharedManagerPtr& shared)
{
    ...
    else if (actionState == ActionState::SelectionReceived)
    {
        ...
        for (auto& pos : selection)
        {
            ...
            for (auto& entity : entities)
            {
                ...
                auto job = makeCarveJob(jobId, jobType, carveEntities, doDrop, fromAbove, entity,
                                        jobRequirements, jobMarkerEntity);
...
void frts::CarveAction::notify(const EventPtr& event, const SharedManagerPtr& shared)
{
...
frts::Action::State frts::CarveAction::stop(const SharedManagerPtr& shared)
{
    assert(shared != nullptr);
